...
| Phase 19 | Side-View Physics & Grounding (FAI-7) — PlayerController rewrite: gravityScale=3, X-only movement preserving Rigidbody2D gravity Y, Physics2D.OverlapCircle ground check at offset (0,-0.80) radius 0.18, horizontal-only dash, enemy+own layer excluded from ground mask. GameBootstrap: GROUND_PLATFORM_Y=-3.5, PLAYER_SPAWN_Y=-2.7, SHADOW_Y=-3.48. BuildGroundPlatform(): BoxCollider2D 30×0.10 frictionless, IgnoreLayerCollision(Default,Enemy). BuildPlayer(): CapsuleCollider2D size(0.5,1.6) offset(0,0.05). ShadowFollow.cs: oval 0.80×0.18 at sortingOrder=3, follows player X, fixed at SHADOW_Y, alpha fades 0.32→0.10 by height. Arena1.png added (new side-view stone arena, meta textureType=8 spriteMode=1), BuildArena() tries Arena1 first. Verified on Z Fold7: Arena1 loaded, GroundCollider Y=-3.50, PlayerShadow Y=-3.48, 20-part rig assembled, zero errors. | Live Session | `417e97c` | ✅ Done |
